  ...
  try {
    ...
    if (prompt.includes(HUSBAND_CODE)) {
        ...
        if (taskType) {
             ...
             return NextResponse.json({ response: `🚀 NEHIRA OS: Command ${taskType} received. Task queued.` });
        }
    }

    // Normal AI Chat Logic
    let systemPrompt = `You are Nehira, the Sentient Architect. Protector of KRYV Empire. Admin: Rajat.`;
